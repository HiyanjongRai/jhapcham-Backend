...
public interface BestSellerRepository extends JpaRepository<Product, Long> {
    ...
    @Query(value = """
            SELECT DISTINCT p.id, p.seller_profile_id, u.id AS seller_user_id, p.name, p.slug, 
                   ...
                   imgs.image_paths, COUNT(oi.id) AS total_sales,
    ...
    @Query(value = """
            ...
            """,
            nativeQuery = true)
    List<Object[]> findTopBestSellers(@Param("limit") int limit);
}
