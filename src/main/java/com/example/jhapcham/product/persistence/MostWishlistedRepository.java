...
public interface MostWishlistedRepository extends JpaRepository<Product, Long> {
    ...
    @Query(value = """
            SELECT DISTINCT p.id, p.seller_profile_id, u.id AS seller_user_id, p.name, p.slug, 
                   ...
                   imgs.image_paths, COUNT(w.id) AS wishlist_count,
            ...
            GROUP BY p.id, p.seller_profile_id, u.id, p.name, p.slug, p.category, p.brand, 
                     ...
                     imgs.image_paths, rv.average_rating, rv.total_reviews, u.full_name, 
                     sp.store_name, sp.logo_image_path, vs.total_stock, vs.min_price, vs.max_price
            ORDER BY wishlist_count DESC, p.id DESC
            """,
            countQuery = """
                    ...
                    """,
            nativeQuery = true)
    Page<Object[]> findMostWishlisted(Pageable pageable);
    ...
    @Query(value = """
            ...
            """,
            nativeQuery = true)
    List<Object[]> findTopMostWishlisted(@Param("limit") int limit);
}
