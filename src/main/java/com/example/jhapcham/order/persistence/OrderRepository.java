...
public interface OrderRepository extends JpaRepository<Order, Long> {
        ...
        @Query("""
                        SELECT o FROM Order o 
                        WHERE o.status <> com.example.jhapcham.order.domain.OrderStatus.DRAFT
                          AND o.id IN (
        ...
        @Query("""
                        ...
                        WHERE i.product.sellerProfile.user.id = :sellerUserId
                          ...
                          AND o.status <> com.example.jhapcham.order.domain.OrderStatus.DRAFT
                        """)
        ...
        @Query("""
                ...
                WHERE o.user.id = :buyerId
                  AND i.product.sellerProfile.user.id = :sellerId
                  AND o.status = com.example.jhapcham.order.domain.OrderStatus.DELIVERED
                """)
        long countDeliveredOrdersByBuyerAndSeller(@Param("buyerId") Long buyerId, @Param("sellerId") Long sellerId);
}
