...
public class ShipmentController {
    ...
    private final com.example.jhapcham.order.OrderRepository orderRepository;
    ...
    private void assertCanCreateShipment(Long orderId, Authentication authentication) {
        ...
        if (actor.getRole() == com.example.jhapcham.user.model.Role.ADMIN) {
            return;
        }
        var order = orderRepository.findById(orderId)
                .orElseThrow(() -> new com.example.jhapcham.Error.ResourceNotFoundException("Order not found"));
        boolean ownsAllItems = actor.getRole() == com.example.jhapcham.user.model.Role.SELLER
