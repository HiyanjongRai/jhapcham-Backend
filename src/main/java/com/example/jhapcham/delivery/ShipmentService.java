...
public class ShipmentService {
    ...
    public DeliveryResponseDTO createShipment(Long orderId) {
        ...
        if (!List.of(
                com.example.jhapcham.order.OrderStatus.PACKED,
                com.example.jhapcham.order.OrderStatus.SHIPPED,
                com.example.jhapcham.order.OrderStatus.OUT_FOR_DELIVERY).contains(order.getStatus())) {
            throw new BusinessValidationException("Shipment can only be created for packed, shipped, or out-for-delivery orders");
        }
        if (order.getPaymentMethod() != PaymentMethod.COD
                && order.getPaymentStatus() != com.example.jhapcham.order.PaymentStatus.PAID) {
        ...
        if (!List.of(
                com.example.jhapcham.order.OrderStatus.SHIPPED,
                com.example.jhapcham.order.OrderStatus.OUT_FOR_DELIVERY,
                com.example.jhapcham.order.OrderStatus.DELIVERED).contains(order.getStatus())) {
